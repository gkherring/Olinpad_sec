...
exports.basicAuth = function (req, res, next) {
  ...
  var authenticate = function (cb) {
    ...
    if (req.headers.authorization && req.headers.authorization.search('Basic ') === 0) {
      console.info("req.headers: " + req.connection.remoteAddress)
      var userpass = new Buffer(req.headers.authorization.split(' ')[1], 'base64').toString().split(":")
      var username = userpass[0];
      var password = userpass[1];
      var date = new Date();

      if (!olinapps.sessionid(req)){
        var data = JSON.stringify({
            "username": username,
            "password": password
        });
        var options = {
          url: 'https://olinapps.herokuapp.com/api/exchangelogin',
          hostname: 'olinapps.herokuapp.com',
          port: 443,
          path: '/api/exchangelogin',
          method: 'POST',
          headers: {
            accept: '*/*',
        ...
        var userinfo = https.request(options, function(res){
          console.log("statusCode: ", res.statusCode);
          console.log("headers: ", res.headers);
          
          res.on('data', function(d){
            console.info("Inside of the data parser"+d);
            var obj = JSON.parse(d);
            if (obj.error == false && res.statusCode == 200 && obj && 'user' in obj){
              req.session['sessionid'] = obj.sessionid;
              req.session['user'] = obj.user;
              settings.users[username].IPaddr = req.connection.remoteAddress;
              settings.users[username].last_auth = date.getTime();
            };
            console.info("set tempuser");
          });
        });
	...
	console.log(userinfo._headers);
        userinfo.on('error', function(e) {
          console.error(e);
        });
      }
//console.info("something "+JSON.stringify(settings.users));
//settings.users[username].password == password
//console.info(util.format("time_amount: %d", date.getTime()-settings.users[username].last_auth))
      if (settings.users[username] != undefined && (date.getTime()-settings.users[username].last_auth < 60000) && settings.users[username].IPaddr == req.connection.remoteAddress) {
